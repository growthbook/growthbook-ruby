...
describe 'util' do
  describe 'checkRule function' do
    it 'works for all operators and normal inputs' do
      # =
      expect(Growthbook::Util.checkRule('test', '=', 'test')).to be(true)
      expect(Growthbook::Util.checkRule('test', '=', 'other')).to be(false)
      # !=
      expect(Growthbook::Util.checkRule('test', '!=', 'other')).to be(true)
      expect(Growthbook::Util.checkRule('test', '!=', 'test')).to be(false)
      # >
      expect(Growthbook::Util.checkRule('b', '>', 'a')).to be(true)
      expect(Growthbook::Util.checkRule('a', '>', 'b')).to be(false)
      # <
      expect(Growthbook::Util.checkRule('a', '<', 'b')).to be(true)
      expect(Growthbook::Util.checkRule('b', '<', 'a')).to be(false)
      # ~
      expect(Growthbook::Util.checkRule('123-456-abc', '~', '^[0-9]{3}-[0-9]{3}-[a-z]{3}$')).to be(true)
      expect(Growthbook::Util.checkRule('123-abc-456', '~', '^[0-9]{3}-[0-9]{3}-[a-z]{3}$')).to be(false)
      # !~
      expect(Growthbook::Util.checkRule('123-abc-456', '!~', '^[0-9]{3}-[0-9]{3}-[a-z]{3}$')).to be(true)
      expect(Growthbook::Util.checkRule('123-456-abc', '!~', '^[0-9]{3}-[0-9]{3}-[a-z]{3}$')).to be(false)
    end

    it "returns true when there's an unknown operator" do
      expect(Growthbook::Util.checkRule('abc', '*', '123')).to be(true)
    end

    it 'returns false when the regex is invalid' do
      expect(Growthbook::Util.checkRule('abc', '~', 'abc)')).to be(false)
    end

    it 'compares numeric strings with natural ordering' do
      expect(Growthbook::Util.checkRule('10', '>', '9')).to be(true)
      expect(Growthbook::Util.checkRule('9', '<', '1000')).to be(true)
      expect(Growthbook::Util.checkRule('90', '>', '800')).to be(false)
      expect(Growthbook::Util.checkRule('-10', '<', '10')).to be(true)
      expect(Growthbook::Util.checkRule('10', '>', 'abc')).to be(false)
    end

    it 'checks for numeric equality properly' do
      expect(Growthbook::Util.checkRule('9.0', '=', '9')).to be(true)
      expect(Growthbook::Util.checkRule('1.3', '!=', '1.30000')).to be(false)
    end

    it 'handles empty strings' do
      expect(Growthbook::Util.checkRule('', '=', '')).to be(true)
      expect(Growthbook::Util.checkRule('', '!=', '')).to be(false)
      expect(Growthbook::Util.checkRule('', '>', '')).to be(false)
      expect(Growthbook::Util.checkRule('', '<', '')).to be(false)
      expect(Growthbook::Util.checkRule('', '~', '')).to be(true)
      expect(Growthbook::Util.checkRule('', '!~', '')).to be(false)
